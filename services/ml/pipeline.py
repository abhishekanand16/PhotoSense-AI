...
class MLPipeline:
    ...
    def __init__(
        ...
        # -------------------------------------------------------------------
        # CPU/threading caps (reduce CPU spikes on i5/M2)
        # Users can override via environment variables set before startup.
        # -------------------------------------------------------------------
        try:
            # OpenCV can spawn many worker threads; keep it modest on CPU-only scans.
            cv2.setNumThreads(1)
        except Exception:
            pass
        try:
            ...
            # Interop threads are usually best kept low on desktop workloads
